...
function DefaultLoader() {
  return (
    <div style={{
      ...
      background: 'rgba(232,232,232,0.6)', backdropFilter: 'blur(4px)',
    }}>
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  // Fallback: hide loader after 8 s even if tiles haven't resolved (offline / slow)
  ...
  useEffect(() => {
    ...
    // Fallback: if load hasn't fired in 8 s (offline / tile error), unblock children
    const fallbackTimer = setTimeout(() => setLoaderTimedOut(true), 8000);
  ...
  const mapReady = isLoaded && isStyleLoaded;

  const contextValue = useMemo(
    () => ({
      map: mapInstance,
      isLoaded: mapReady || loaderTimedOut,
    }),
    [mapInstance, mapReady, loaderTimedOut],
  );
  ...
  return (
    <MapContext.Provider value={contextValue}>
      ...
      >
        {/* Show loader until tiles loaded OR fallback timeout hit */}
        {(!mapReady && !loaderTimedOut && !loading) && <DefaultLoader />}
        {/* Render children once canvas exists (markers need the DOM, not tile network) */}
