...
} from '\var\task\apps\frontend/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\apps\frontend/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\apps\frontend')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\apps\frontend',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\apps\frontend\.netlify\dist\run\handlers\server.js')
