...
interface AuthContextType {
  ...
  // Delivery location (set by LocationModal)
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // ── refreshUser (re-read Firebase state + local profile) ──────────────────────
  const refreshUser = async () => {
    if (!isFirebaseReady || !auth) return;
    // Force-reload the Firebase user token so displayName / claims are fresh
    if (auth.currentUser) {
